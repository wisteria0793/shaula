...
### 開発手順
1. 施設情報の登録・変更・削除(CRUD)機能
- バックエンド
    - Facilityモデルを定義
    - 施設情報を操作するためのAPIを作成
- フロントエンド
    - 施設一覧ページ、登録ページ、編集ページを作成
    - バックエンドAPIと連携し、操作可能にする

2. 宿泊者名簿と宿泊税支払い状況の管理機能
- バックエンド
    - Guestモデルを定義
    - Google sheets APIと連携し、名簿の提出を管理
    - 宿泊税の支払いを管理するフィールドをモデルに追加
...
3. 民泊の売り上げ機能
- バックエンド
    - Beds24と連携し、売り上げを集計するロジックを実装
    - 月別や施設別などで集計した売り上げデータを返すAPIの作成

- フロントエンド
    - 売り上げデータを表示するダッシュボードページを作成
    - グラフ描画ライブラリを導入し、視覚的に表示
---
